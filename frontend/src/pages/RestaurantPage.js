...
const RestaurantPage = () => {
  ...
  useEffect(() => {
    const fetchRestaurantData = async () => {
      try {
        const restaurantResponse = await axios.get(`http://localhost:10000/restaurant/profile/${restaurantId}`);
    ...
    const fetchMenuData = async () => {
      try {
        const menuResponse = await axios.get(`http://localhost:10000/restaurant/menu/${restaurantId}`);
    ...
    const fetchCategories = async () => {
      try {
        const categoriesResponse = await axios.get('http://localhost:10000/restaurant/categories');
